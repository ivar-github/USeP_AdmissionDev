<div class="max-w-2xl  w-full h-full border-2 dark:border-gray-500 rounded-lg shadow dark:bg-gray-800 p-5">
    <div class="flex justify-between border-b border-gray-200 dark:border-gray-500 pb-2">
        <div class="flex justify-center items-center">
            <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white pe-1">Applicants</h5>
    ...
    <!-- Donut Chart -->
    <div class="py-6" id="student-pie"></div>
...
@push('scripts')
    ...
    <script>
       document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderStudentPie(data) {
            
            const qualifiedDatab = data.map(item => ({ x: `Term ${item.TermID}`, y: item.appQualified }));
            const wavedslotDatab = data.map(item => ({ x: `Term ${item.TermID}`, y: item.appWaivedSlot }));
            const confirmedDatab = data.map(item => ({ x: `Term ${item.TermID}`, y: item.appConfirmed }));
            const totalDatab = data.map(item => ({ x: `Term ${item.TermID}`, y: item.appTotal }));

            console.log('qualifiedData', qualifiedDatab);
            console.log('wavedslotData', wavedslotDatab);
            console.log('confirmedData', confirmedDatab);
            console.log('totalData', totalDatab);
            ...
            if (document.getElementById("student-pie") && typeof ApexCharts !== 'undefined') {
                const chart = new ApexCharts(document.getElementById("student-pie"), option);
                chart.render();
