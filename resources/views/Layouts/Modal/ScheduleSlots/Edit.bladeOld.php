        ...
        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
            ...
            <form method="POST" id="editExamineeSchedForm">
                ...
                <div class="grid grid-cols-1 mt-4 gap-4">
                    <div class=" text-white">
                        <x-Input-Label for="e_slots">Available Slots</x-Input-Label>
                        <x-Input-Dropdown name="Slots" id="e_slots">
                            <option value="" disabled selected>Select</option>
                            {{-- @foreach ($ScheduleSlots as $ScheduleSlot)
                                <option value="{{ $ScheduleSlot->id }}">
                                    {{ $ScheduleSlot->testCenterName }}
                                    {{ $ScheduleSlot->testDate }} {{ $ScheduleSlot->testTimeStartString }}-{{ $ScheduleSlot->testTimeEndString }}  
                                    ({{ $ScheduleSlot->testRoomName }}) [{{ $ScheduleSlot->totalRegistered }} / {{ $ScheduleSlot->availableSlots}}]
                                </option>
                            @endforeach --}}
                        </x-Input-Dropdown>
                    </div>
